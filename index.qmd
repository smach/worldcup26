...
matches_for_js <- matches |>
  ...
  # The JS does its own EDT formatting from utc_date_str; drop the R-side
  # datetime/Eastern columns so a Date doesn't round-trip through ojs_define().
  select(-utc_date, -match_date, -kickoff_edt)
...
*Last rebuilt: `r format(Sys.time(), "%B %e, %Y at %H:%M EDT", tz = "America/New_York")`*
:::

*All times shown in US Eastern Time (EDT).*
...
// All times are shown in US Eastern (EDT); the World Cup runs entirely within
// EDT and every venue is in North America.
nyDayFmt = new Intl.DateTimeFormat("en-CA", {timeZone: "America/New_York"})
function nyDayKey(d) { return nyDayFmt.format(d); }   // "2026-06-12" in Eastern
...
function formatTime(d) {
  return d.toLocaleTimeString("en-US", {hour: "numeric", minute: "2-digit", timeZone: "America/New_York"}) + " EDT";
}
function formatDayHeading(isoDay) {
  // isoDay is already the Eastern calendar date; format it as UTC midday so the
  // weekday/month/day read back out without a second timezone shift.
